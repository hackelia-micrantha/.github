...
Live full-inventory planning must not trust one offset-paginated pass. The report/planner requires two consecutive normalized complete label inventories to match, retries within a fixed bound when they differ, and fails closed if the inventory does not stabilize. Future preflight/apply/finalizer full-plan regeneration must preserve that stabilized-read invariant; the immediate requested-label operation precondition remains a separate final read.
...
The receipt/finalizer has no write permission. When apply starts or mutation outcome is not conclusively known, it must independently re-read live `main` and selected label state. If a runner is lost/cancelled after a create request may have been sent and the response is unavailable, the final result is `indeterminate-mutation`, not `failed` or `not-applied`; it sets `requiresDisposition: true` and preserves attribution ambiguity. If live inventory cannot stabilize during reconciliation, the finalizer must fail closed into uncertainty/quarantine rather than infer state. If the whole workflow ends without a terminal receipt after apply may have started, that absence itself is quarantine evidence.
