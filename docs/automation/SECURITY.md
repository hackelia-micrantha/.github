...
The protected environment is also the first-pilot quarantine gate. Its accountable reviewer must reject a new apply while any prior `applied-with-race` receipt lacks durable reviewed disposition evidence. The immutable dispatch carries `race_disposition_ref`; `none` is valid only when the reviewer verifies no unresolved race. A fresh revision or plan alone cannot clear quarantine.

The receipt/finalizer has no write permission and must preserve terminal evidence even when environment approval is rejected, the apply job is cancelled, or the write-authorized job never starts. It distinguishes job state from whether mutation actually started or completed and marks raced creates as requiring disposition.
