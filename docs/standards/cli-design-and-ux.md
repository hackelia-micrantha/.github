...
The caller is responsible for confirming the selected command is genuinely read-only; a profile is test input, **not** authority to execute an effectful operation. `executable[0]` must resolve to an executable file inside `install_root`, so the probe cannot claim installation evidence while actually running a development checkout or unrelated host binary. Interpreter-based packages should install a wrapper/entry-point executable under the package root rather than pointing the probe directly at a host interpreter. The probe emits a JSON check report and exits nonzero on failed checks, and machine output must be strict JSON (nonstandard constants such as `NaN` or `Infinity` fail). Its profile can customize `help_args` and `version_args` for established interfaces. The version above is an example, not a released repoctl version.
